...
def modify_admin(
    ...
    if (dbadmin.username != current_admin.username) and dbadmin.is_sudo:
        raise HTTPException(
            status_code=403,
            detail="You're not allowed to edit another sudoer's account. Use the darknight CLI instead.",
        )
...
def remove_admin(
    ...
    if dbadmin.is_sudo:
        raise HTTPException(
            status_code=403,
            detail="You're not allowed to delete sudo accounts. Use the darknight CLI instead.",
        )
