...
from darknight import telegram
from darknight.db import Session, create_notification_reminder, get_admin_by_id, GetDB
from darknight.db.models import UserStatus, User
from darknight.models.admin import Admin
from darknight.models.user import ReminderType, UserResponse
from darknight.utils.notification import (Notification, ReachedDaysLeft,
...
def expire_days_reached(db: Session, days: int, user: UserResponse, user_id: int, expire: int, threshold=None) -> None:
    ...
    if get_app_config().notifications.if_days_left_reached:
...
def login(username: str, password: str, client_ip: str, success: bool) -> None:
    if get_app_config().notifications.login:
        try:
