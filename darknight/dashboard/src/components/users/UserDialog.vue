  ...
  selected_proxies: [
    {
      validator: (_r, value: ProxyKey[], cb) =>
        value && value.length > 0
          ? cb()
          : cb(new Error(t("userDialog.selectOneProtocol"))),
      trigger: "change",
    },
  ],
};
...
function loadUser(user: User) {
  ...
  form.proxies.shadowsocks.method =
    user.proxies.shadowsocks?.method ?? "chacha20-ietf-poly1305";
...
function buildProxies(): UserCreate["proxies"] {
  ...
  for (const key of form.selected_proxies) {
    ...
    if (key === "trojan")
      result.trojan = form.proxies.trojan.password
        ? { password: form.proxies.trojan.password }
        : {};
    if (key === "shadowsocks")
      result.shadowsocks = {
        ...(form.proxies.shadowsocks.password
          ...
          : {}),
        ...(form.proxies.shadowsocks.method
          ? { method: form.proxies.shadowsocks.method }
          : {}),
      };
  }
  return result;
...
async function onSubmit() {
  ...
  errorMsg.value = "";
  const dataLimitBytes = form.data_limit
    ? Number((form.data_limit * GB).toFixed(0))
    : 0;
...
<template>
  ...
  >
    <el-form ref="formRef" :model="form" :rules="rules" label-position="top">
      ...
      <el-form-item
        v-if="form.selected_proxies.includes('vless')"
        label="VLESS flow"
      >
