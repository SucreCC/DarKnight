...
export function useUsersQuery(filters: MaybeRefOrGetter<UserFilters>) {
  return useQuery({
    ...
    queryFn: () =>
      http<UsersResponse>("/users", { query: cleanFilters(toValue(filters)) }),
...
export function useUserMutations() {
  ...
  const createUser = useMutation({
    mutationFn: (body: UserCreate) =>
      http<User>("/user", { method: "POST", body }),
    onSuccess: invalidate,
  });

  const updateUser = useMutation({
    mutationFn: (body: UserCreate) =>
      http<User>(`/user/${body.username}`, { method: "PUT", body }),
    onSuccess: invalidate,
  });

  const deleteUser = useMutation({
    mutationFn: (username: string) =>
      http(`/user/${username}`, { method: "DELETE" }),
    onSuccess: invalidate,
  });

  const resetUserUsage = useMutation({
    mutationFn: (username: string) =>
      http(`/user/${username}/reset`, { method: "POST" }),
    onSuccess: invalidate,
  });
