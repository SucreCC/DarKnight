#!/usr/bin/env bash
# DarKnight-branded panel install script asset (renamed from the legacy panel installer).
# NOT the official installer for the current DarKnight main panel —
# deploy the panel with E:\kai\DarKnight docker compose instead.
set -e
...
install_darknight_script() {
    colorized_echo blue "Installing darknight script from local file"
    install -m 755 "$SCRIPT_SOURCE" /usr/local/bin/darknight
    colorized_echo green "darknight script installed successfully"
}

is_darknight_installed() {
    if [ -d $APP_DIR ]; then
        return 0
    else
        return 1
...
send_backup_to_telegram() {
    ...
    local split_dir="/tmp/darknight_backup_split"
...
add_cron_job() {
    ...
    echo "$schedule $command # darknight-backup-service" >> "$temp_cron"
...
remove_backup_service() {
    ...
    sed -i '/# darknight-backup-service/d' "$temp_cron"
...
backup_command() {
    ...
    local temp_dir="/tmp/darknight_backup"
    local timestamp=$(date +"%Y%m%d%H%M%S")
    local backup_file="$backup_dir/backup_$timestamp.tar.gz"
    local error_messages=()
    local log_file="/var/log/darknight_backup_error.log"
    > "$log_file"
    ...
    if [ -n "$db_type" ]; then
        ...
        case $db_type in
            mariadb)
                ...
                ;;
            mysql)
                if ! docker exec "$container_name" mysqldump -u root -p"$MYSQL_ROOT_PASSWORD" darknight --events --triggers  > "$temp_dir/db_backup.sql" 2>>"$log_file"; then
    ...
    rsync -av --exclude 'xray-core' --exclude 'mysql' "$DATA_DIR/" "$temp_dir/darknight_data/" >>"$log_file" 2>&1
...
# Function to update the DarKnight Main core
update_core_command() {
    check_running_as_root
    get_xray_core
    # Change the DarKnight core
    xray_executable_path="XRAY_EXECUTABLE_PATH=\"/var/lib/darknight/xray-core/xray\""
    
    echo "Changing the DarKnight core..."
    ...
    # Restart DarKnight
    colorized_echo red "Restarting DarKnight..."
    if restart_command -n >/dev/null 2>&1; then
        colorized_echo green "DarKnight successfully restarted!"
    else
        colorized_echo red "DarKnight restart failed!"
    fi
    colorized_echo blue "Installation of Xray-core version $selected_version completed."
}

install_darknight() {
    local darknight_version=$1
    local database_type=$2
    # Fetch releases
    FILES_URL_PREFIX="https://raw.githubusercontent.com/DarKnight/DarKnight/master"
...
services:
  darknight:
    image: darknight:${darknight_version}
    restart: always
    env_file: .env
    network_mode: host
    volumes:
      - /var/lib/darknight:/var/lib/darknight
      - /var/lib/darknight/logs:/var/lib/darknight-node
    depends_on:
      mariadb:
  ...
  mariadb:
    ...
    volumes:
      - /var/lib/darknight/mysql:/var/lib/mysql
    healthcheck:
...
EOF
        ...
        sed -i 's~^\(SQLALCHEMY_DATABASE_URL = "sqlite:////var/lib/darknight/db.sqlite3"\)~#\1~' "$APP_DIR/.env"


        # Add the MySQL connection string
        #echo -e '\nSQLALCHEMY_DATABASE_URL = "mysql+pymysql://darknight:password@127.0.0.1:3306/darknight"' >> "$APP_DIR/.env"

        sed -i 's/^# \(XRAY_JSON = .*\)$/\1/' "$APP_DIR/.env"
        sed -i 's~\(XRAY_JSON = \).*~\1"/var/lib/darknight/xray_config.json"~' "$APP_DIR/.env"


        prompt_for_darknight_password
        ...
        echo "MYSQL_DATABASE=darknight" >> "$ENV_FILE"
        echo "MYSQL_USER=darknight" >> "$ENV_FILE"
        echo "MYSQL_PASSWORD=$MYSQL_PASSWORD" >> "$ENV_FILE"
        
        SQLALCHEMY_DATABASE_URL="mysql+pymysql://darknight:${MYSQL_PASSWORD}@127.0.0.1:3306/darknight"
...
services:
  ...
  mysql:
    ...
    volumes:
      - /var/lib/darknight/mysql:/var/lib/mysql
    healthcheck:
      test: ["CMD", "mysqladmin", "ping", "-h", "127.0.0.1", "-u", "darknight", "--password=\${MYSQL_PASSWORD}"]
      start_period: 5s
      interval: 5s
      timeout: 5s
      retries: 55
...
EOF
        ...
        if [ "$darknight_version" == "latest" ]; then
            yq -i '.services.darknight.image = "darknight:latest"' "$docker_file_path"
        else
            yq -i ".services.darknight.image = \"darknight:${darknight_version}\"" "$docker_file_path"
        fi
        echo "Installing $darknight_version version"
        ...
        sed -i 's~\(XRAY_JSON = \).*~\1"/var/lib/darknight/xray_config.json"~' "$APP_DIR/.env"
        sed -i 's~\(SQLALCHEMY_DATABASE_URL = \).*~\1"sqlite:////var/lib/darknight/db.sqlite3"~' "$APP_DIR/.env"
    ...
    colorized_echo green "DarKnight's files downloaded successfully"
}
...
follow_darknight_logs() {
    $COMPOSE -f $COMPOSE_FILE -p "$APP_NAME" logs -f
}

status_command() {
    
    # Check if darknight is installed
    if ! is_darknight_installed; then
        echo -n "Status: "
...
prompt_for_darknight_password() {
    ...
    read -p "Enter the password for the darknight user (or press Enter to generate a secure default password): " MYSQL_PASSWORD
...
install_command() {
    ...
    darknight_version="latest"
    darknight_version_set="false"
    ...
    while [[ $# -gt 0 ]]; do
        ...
        case $key in
            ...
            ;;
            --dev)
                if [[ "$darknight_version_set" == "true" ]]; then
                    colorized_echo red "Error: Cannot use --dev and --version options simultaneously."
                    exit 1
                fi
                darknight_version="dev"
                darknight_version_set="true"
                shift
            ;;
            --version)
                if [[ "$darknight_version_set" == "true" ]]; then
    ...
    # Check if darknight is already installed
    if is_darknight_installed; then
        colorized_echo red "DarKnight is already installed at $APP_DIR"
    ...
    check_version_exists() {
        local version=$1
        repo_url="https://api.github.com/repos/DarKnight/DarKnight/releases"
    ...
    if [[ "$darknight_version" == "latest" || "$darknight_version" == "dev" || "$darknight_version" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        if check_version_exists "$darknight_version"; then
            install_darknight "$darknight_version" "$database_type"
            echo "Installing $darknight_version version"
        else
            echo "Version $darknight_version does not exist. Please enter a valid version (e.g. v0.5.2)"
            exit 1
        fi
    else
...
darknight_cli() {
    $COMPOSE -f $COMPOSE_FILE -p "$APP_NAME" exec -e CLI_PROG_NAME="darknight cli" darknight darknight-cli "$@"
}
...
uninstall_command() {
    ...
    # Check if darknight is installed
    if ! is_darknight_installed; then
        colorized_echo red "DarKnight's not installed!"
        exit 1
    fi
    
    read -p "Do you really want to uninstall DarKnight? (y/n) "
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    ...
    uninstall_darknight_script
    uninstall_darknight
    uninstall_darknight_docker_images
    
    read -p "Do you want to remove DarKnight's data files too ($DATA_DIR)? (y/n) "
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        colorized_echo green "DarKnight uninstalled successfully"
    else
        uninstall_darknight_data_files
        colorized_echo green "DarKnight uninstalled successfully"
    fi
}

uninstall_darknight_script() {
    if [ -f "/usr/local/bin/darknight" ]; then
        colorized_echo yellow "Removing darknight script"
        rm "/usr/local/bin/darknight"
    fi
}
...
uninstall_darknight_docker_images() {
    images=$(docker images | grep darknight | awk '{print $3}')
    
    if [ -n "$images" ]; then
        colorized_echo yellow "Removing Docker images of DarKnight"
...
restart_command() {
    help() {
        colorized_echo red "Usage: darknight restart [options]"
        echo
        echo "OPTIONS:"
...
logs_command() {
    help() {
        colorized_echo red "Usage: darknight logs [options]"
        echo ""
        echo "OPTIONS:"
        echo "  -h, --help        display this help message"
    ...
    if ! is_darknight_up; then
        colorized_echo red "DarKnight is not up."
        exit 1
    fi
...
down_command() {
    ...
    if ! is_darknight_up; then
        colorized_echo red "DarKnight's already down"
        exit 1
    fi
...
up_command() {
    help() {
        colorized_echo red "Usage: darknight up [options]"
        echo ""
        echo "OPTIONS:"
        echo "  -h, --help        display this help message"
        echo "  -n, --no-logs     do not follow logs after starting"
    ...
    if is_darknight_up; then
        colorized_echo red "DarKnight's already up"
        exit 1
    fi
...
update_command() {
    ...
    update_darknight_script
    colorized_echo blue "Pulling latest version"
    update_darknight
    
    colorized_echo blue "Restarting DarKnight's services"
    down_darknight
    up_darknight
    
    colorized_echo blue "DarKnight updated successfully"
}

update_darknight_script() {
    colorized_echo blue "Updating darknight script from local file"
    if [ ! -f "$SCRIPT_SOURCE" ]; then
        colorized_echo red "Local script not found at $SCRIPT_SOURCE"
        return 1
    fi
    install -m 755 "$SCRIPT_SOURCE" /usr/local/bin/darknight
    colorized_echo green "darknight script updated successfully"
}
...
usage() {
    local script_name="${0##*/}"
    colorized_echo blue "=============================="
    colorized_echo magenta "           DarKnight Help"
    colorized_echo blue "=============================="
    ...
    colorized_echo yellow "  cli             $(tput sgr0)– DarKnight CLI"
    colorized_echo yellow "  install         $(tput sgr0)– Install DarKnight"
    colorized_echo yellow "  update          $(tput sgr0)– Update to latest version"
    colorized_echo yellow "  uninstall       $(tput sgr0)– Uninstall DarKnight"
    colorized_echo yellow "  install-script  $(tput sgr0)– Install DarKnight script"
    colorized_echo yellow "  backup          $(tput sgr0)– Manual backup launch"
    colorized_echo yellow "  backup-service  $(tput sgr0)– DarKnight Backupservice to backup to TG, and a new job in crontab"
...
case "$1" in
    ...
    install-script)
        shift; install_darknight_script "$@";;
    core-update)
