...
elif [ -d "$INSTALL_DIR/DarKnight-node" ]; then
    APP_DIR="$INSTALL_DIR/DarKnight-node"
else
    APP_DIR="$INSTALL_DIR/$APP_NAME"
fi
...
# Install/update the CLI from this local script file (no remote script fetch).
...
install_darknight_node_script() {
    colorized_echo blue "Installing darknight-node script"
    TARGET_PATH="/usr/local/bin/$APP_NAME"
    install -m 755 "$SCRIPT_SOURCE" "$TARGET_PATH"

    sed -i "s/^APP_NAME=.*/APP_NAME=\"$APP_NAME\"/" $TARGET_PATH

    colorized_echo green "DarKnight-node script installed successfully at $TARGET_PATH"
}
...
services:
  darknight-node:
    ...
    build: .
    image: darknight-node:latest
    restart: always
    network_mode: host
    environment:
      SSL_CLIENT_CERT_FILE: "/var/lib/darknight-node/cert.pem"
...
EOL
    ...
    volumes:
      - $DATA_DIR:/var/lib/darknight-node
EOL
    colorized_echo green "File saved in $APP_DIR/docker-compose.yml"
}


uninstall_darknight_node_script() {
    if [ -f "/usr/local/bin/$APP_NAME" ]; then
        colorized_echo yellow "Removing darknight-node script"
        rm "/usr/local/bin/$APP_NAME"
    fi
}

uninstall_darknight_node() {
    if [ -d "$APP_DIR" ]; then
...
uninstall_darknight_node_docker_images() {
    images=$(docker images | grep darknight-node | awk '{print $3}')
    
    if [ -n "$images" ]; then
        colorized_echo yellow "Removing Docker images of DarKnight-node"
...
uninstall_darknight_node_data_files() {
    if [ -d "$DATA_DIR" ]; then
...
show_darknight_node_logs() {
    $COMPOSE -f $COMPOSE_FILE -p "$APP_NAME" logs
}

follow_darknight_node_logs() {
    $COMPOSE -f $COMPOSE_FILE -p "$APP_NAME" logs -f
}

update_darknight_node_script() {
    colorized_echo blue "Updating darknight-node script from local file"
    if [ ! -f "$SCRIPT_SOURCE" ]; then
        colorized_echo red "Local script not found at $SCRIPT_SOURCE"
        colorized_echo yellow "Re-run from the DarKnightNode repository: sudo bash scripts/darknight-node.sh install-script"
        return 1
    fi
    install -m 755 "$SCRIPT_SOURCE" "/usr/local/bin/$APP_NAME"
    colorized_echo green "darknight-node script updated successfully"
}

update_darknight_node() {
    $COMPOSE -f $COMPOSE_FILE -p "$APP_NAME" build --pull
}

is_darknight_node_installed() {
    if [ -d $APP_DIR ]; then
        return 0
    else
        return 1
...
install_command() {
    ...
    if is_darknight_node_installed; then
        colorized_echo red "DarKnight-node is already installed at $APP_DIR"
    ...
    up_darknight_node
    follow_darknight_node_logs
    echo "Use your IP: $NODE_IP and defaults ports: $SERVICE_PORT and $XRAY_API_PORT to setup your DarKnight Main Panel"
}
...
uninstall_command() {
    ...
    if ! is_darknight_node_installed; then
        colorized_echo red "DarKnight-node not installed!"
        exit 1
    fi
    
    read -p "Do you really want to uninstall DarKnight-node? (y/n) "
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    ...
    read -p "Do you want to remove DarKnight-node data files too ($DATA_DIR)? (y/n) "
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        colorized_echo green "DarKnight-node uninstalled successfully"
    else
        uninstall_darknight_node_data_files
        colorized_echo green "DarKnight-node uninstalled successfully"
    fi
...
up_command() {
    help() {
        colorized_echo red "Usage: darknight-node up [options]"
        echo ""
        echo "OPTIONS:"
    ...
    # Check if darknight-node is installed
    if ! is_darknight_node_installed; then
        colorized_echo red "DarKnight-node's not installed!"
        exit 1
    fi
    ...
    if is_darknight_node_up; then
        colorized_echo red "DarKnight-node's already up"
        exit 1
    fi
...
down_command() {
    ...
    if ! is_darknight_node_up; then
        colorized_echo red "DarKnight-node already down"
        exit 1
    fi
...
restart_command() {
    help() {
        colorized_echo red "Usage: darknight-node restart [options]"
        echo
        echo "OPTIONS:"
        echo "  -h, --help        display this help message"
        echo "  -n, --no-logs     do not follow logs after starting"
...
logs_command() {
    help() {
        colorized_echo red "Usage: darknight-node logs [options]"
        echo ""
        echo "OPTIONS:"
        echo "  -h, --help        display this help message"
    ...
    if ! is_darknight_node_up; then
        colorized_echo red "DarKnight-node is not up."
        exit 1
    fi
...
update_command() {
    ...
    colorized_echo blue "Restarting DarKnight-node services"
    down_darknight_node
    up_darknight_node
    
    colorized_echo blue "DarKnight-node updated successfully"
}
...
update_core_command() {
    ...
    if ! grep -q 'XRAY_EXECUTABLE_PATH: "/var/lib/darknight-node/xray-core/xray"' "$COMPOSE_FILE"; then
        yq eval '.services."darknight-node".environment.XRAY_EXECUTABLE_PATH = "/var/lib/darknight-node/xray-core/xray"' -i "$COMPOSE_FILE"
    fi

    if ! yq eval ".services.\"darknight-node\".volumes[] | select(. == \"${DATA_MAIN_DIR}:/var/lib/darknight-node\")" "$COMPOSE_FILE" &>/dev/null; then
        yq eval ".services.\"darknight-node\".volumes += \"${DATA_MAIN_DIR}:/var/lib/darknight-node\"" -i "$COMPOSE_FILE"
    fi
    
    # Restart DarKnight-node
    colorized_echo red "Restarting DarKnight-node..."
    $APP_NAME restart -n
...
usage() {
    ...
    colorized_echo yellow "  install         $(tput sgr0)– Install/reinstall DarKnight-node"
    colorized_echo yellow "  update          $(tput sgr0)– Update to latest version"
    colorized_echo yellow "  uninstall       $(tput sgr0)– Uninstall DarKnight-node"
    colorized_echo yellow "  install-script  $(tput sgr0)– Install DarKnight-node script"
    colorized_echo yellow "  uninstall-script  $(tput sgr0)– Uninstall DarKnight-node script"
